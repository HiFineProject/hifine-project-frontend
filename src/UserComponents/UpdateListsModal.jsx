...
const UpdateListsModal = ({ setEditModal, setReload, editedList,  }) => {
  ...
  useEffect(() => {
    if (editedList) {
      ...
      const time = editedList.dateTime?.split(' ')[1]?.split(':') || ['00', '00']; // Check if dateTime and time are defined before splitting
      setHour(time[0]);
      setMinute(time[1]);
    }
  }, [editedList]);
  ...
  const handleButtonClick = async () => {
    try {
      console.log('List ID:', editedList._id); // Log the listId before updating the list
      ...
      const response = await axios.patch(`http://127.0.0.1:3000/lists/${editedList._id}`, updatedList);
      ...
      if (response.status === 200 || response.status === 201) {
        ...
        console.error('Failed to update list. Unexpected status code:', response.status);
      }
    } catch (error) {
